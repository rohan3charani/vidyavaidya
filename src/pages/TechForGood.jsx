  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col">
      ...
      <main className="flex-grow">
        <section className="relative pt-32 pb-24 px-6 lg:px-20 overflow-hidden bg-gradient-to-br from-indigo-700 via-purple-800 to-slate-900 text-white text-center">
          ...
          <div className="relative z-10">
            <span className="inline-flex rounded-full bg-indigo-500/20 px-4 py-2 text-sm font-semibold uppercase tracking-[0.24em] text-indigo-200 mb-6">Corporate Network</span>
            <h1 className="text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl mb-6">Tech For Good Inc.</h1>
            <p className="mx-auto max-w-2xl text-base text-indigo-50 sm:text-xl font-medium italic mt-4">Discover the innovative tech companies operating under our Tech For Good partnership.</p>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {sponsors.map(p => (
              <Link to={p.link} key={p.id} className="group flex flex-col items-center p-8 bg-white rounded-3xl shadow-sm border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-indigo-50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
                ...
                <div className="mt-auto pt-4 flex items-center justify-between w-full border-t border-slate-100">
                  <span className="text-xs font-bold uppercase tracking-widest text-indigo-600 bg-indigo-50 px-4 py-1.5 rounded-full">{p.type}</span>
                  <span className="text-indigo-500 font-bold group-hover:translate-x-1 transition-transform duration-300 flex items-center gap-1">
