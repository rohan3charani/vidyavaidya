  ...
  useEffect(() => {
    const fetchHospitals = async () => {
      try {
        ...
        const filtered = (data || []).filter(p => p.type === "hospital" && p.isActive !== false);
      ...
      <main className="flex-grow">
        <section className="relative pt-32 pb-24 px-6 lg:px-20 overflow-hidden bg-gradient-to-br from-green-600 via-teal-700 to-blue-900 text-white text-center">
          ...
          <div className="relative z-10">
            <span className="inline-flex rounded-full bg-emerald-500/20 px-4 py-2 text-sm font-semibold uppercase tracking-[0.24em] text-emerald-200 mb-6">Partner Network</span>
            <h1 className="text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl mb-6">Global Health Care</h1>
            <p className="mx-auto max-w-2xl text-base text-teal-50 sm:text-xl font-medium italic mt-4">Discover the incredible hospitals and medical centers operating under our Global Health Care partnership.</p>
          </div>
        </section>
        ...
        <section className="py-20 px-6 lg:px-20 max-w-7xl mx-auto">
          {loading ? (
            ...
            <div className="text-center py-12 bg-white rounded-3xl border border-slate-100 p-8 shadow-sm max-w-md mx-auto">
              ...
              <h3 className="text-xl font-bold text-slate-800 mb-2">No Hospitals Registered</h3>
              <p className="text-slate-500 text-sm">We are currently expanding our hospital network. Please check back soon!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {hospitals.map(p => (
                <Link to={`/partners/${p.slug}`} key={p.id} className="group flex flex-col items-center p-8 bg-white rounded-3xl shadow-sm border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 relative overflow-hidden">
                  ...
                  {p.logoUrl ? (
                    <img src={p.logoUrl} alt={p.name} className="h-28 w-auto object-contain mb-8 group-hover:scale-110 transition-transform duration-500" />
                  ) : (
