...
const staticImages = Object.values(articleImages).map((module) => module.default);
...
export default function News() {
  ...
  const displayItems = stories.length > 0 
    ? stories.map((s, idx) => ({
        src: s.coverImageUrl || "https://images.unsplash.com/photo-1504711434969-e33886168f5c?auto=format&fit=crop&w=800&q=80",
        title: s.title,
        excerpt: s.excerpt || s.content?.substring(0, 120) + "..."
      }))
    : staticImages.map((src, idx) => ({
        src,
        title: `Archive Feature ${idx + 1}`,
        excerpt: "VidyaVaidya news and media announcement."
      }));

  const midPoint = Math.ceil(displayItems.length / 2);
  const firstHalf = displayItems.slice(0, midPoint);
  const secondHalf = displayItems.slice(midPoint);
                ...
                {firstHalf.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-teal-900/0 group-hover:bg-teal-900/20 transition-colors duration-300 flex items-center justify-center">
                      <span className="opacity-0 group-hover:opacity-100 text-white font-medium px-4 py-2 bg-black/50 rounded-full backdrop-blur-sm transition-opacity duration-300 shadow-lg transform scale-90 group-hover:scale-100 text-center text-xs">
                        {item.title}
                      </span>
                    </div>
                  </div>
                  ...
                  {secondHalf.map((item, index) => (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6">
                        <p className="text-white font-bold text-lg drop-shadow-md line-clamp-2">{item.title}</p>
      ...
      {selectedIndex !== null && (
        ...
        >
          ...
          >
            ...
            {/* Main Image */}
            <img 
              src={displayItems[selectedIndex].src} 
              alt={displayItems[selectedIndex].title} 
